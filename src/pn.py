...
class GraphSearchPolicy(nn.Module):
    ...
    def transit(self, e, obs, kg, use_action_space_bucketing=True, merge_aspace_batching_outcome=False):
        ...
        #print ("batch_num_1: " + str(e_s[:1000]))
        # print ("batch_num_2: " + str(q[:3]))
        # print ("batch_num_3: " + str(e_t[:3]))

        # Representation of the current state (current node and other observations)
        #Q = kg.get_relation_embeddings(q)
        Q_pre = kg.get_relation_embeddings_pretrain(q)
        #Q = torch.cat([Q, Q_pre], dim=-1)
        #Q = F.relu(self.W_R(Q))
        ...
        if self.relation_only:
            ...
            #E_s_pre = kg.get_entity_embeddings_pretrain(e_s)
            E_s = kg.het_gnn(e_s)
            #E = kg.neighbor_encoder(e)
            #E_pre = kg.get_entity_embeddings_pretrain(e)
            E = kg.het_gnn(e)
            #X = torch.cat([E, H, E_s, Q], dim=-1)
            X = torch.cat([E, H, E_s, Q_pre], dim=-1)
        else:
            #E = kg.get_entity_embeddings(e)
            #E = kg.neighbor_encoder(e)
            #E_pre = kg.get_entity_embeddings_pretrain(e)
            E = kg.het_gnn(e)
            #E = torch.cat([E, E_pre], dim=-1)
            #E = F.relu(self.W_E(E))
            #X = torch.cat([E, H, Q], dim=-1)
            X = torch.cat([E, H, Q_pre], dim=-1)
    ...
    def get_action_embedding(self, action, kg):
        ...
        relation_embedding_pre = kg.get_relation_embeddings_pretrain(r)
        #relation_embedding = torch.cat([relation_embedding, relation_embedding_pre], dim=-1)
        #relation_embedding = F.relu(self.W_R(relation_embedding))
        ...
        if self.relation_only:
            ...
            entity_embedding = kg.het_gnn(e)
            #entity_embedding_pre = kg.get_entity_embeddings_pretrain(e)
            
            #E = kg.het_gnn(e)
            #entity_embedding = torch.cat([entity_embedding, entity_embedding_pre], dim=-1)
            #entity_embedding = F.relu(self.W_E(entity_embedding))

            #action_embedding = torch.cat([relation_embedding, entity_embedding], dim=-1)
            action_embedding = torch.cat([relation_embedding_pre, entity_embedding], dim=-1)
    ...
    def define_modules(self):
        ...
        #self.W_R = nn.Linear(self.relation_dim * 2, self.relation_dim)
        #self.W_E = nn.Linear(self.entity_dim * 2, self.entity_dim)
