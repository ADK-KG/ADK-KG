...
# print (args.TransE_state_dict_path)

import data_utils as data_utils
import eval
from hyperparameter_range import hp_range
from knowledge_graph import KnowledgeGraph
from fact_network import ComplEx, ConvE, DistMult, TransE
from fact_network import get_conve_kg_state_dict, get_complex_kg_state_dict, get_distmult_kg_state_dict, get_TransE_kg_state_dict
from emb import EmbeddingBasedMethod
from pn import GraphSearchPolicy
from pg import PolicyGradient
from rs_pg import RewardShapingPolicyGradient
...
def process_data():
    ...
    raw_kb_path = os.path.join(data_dir, 'raw.kb.txt')
    train_path = data_utils.get_train_path(args)
    dev_path = os.path.join(data_dir, 'dev.triples.txt')
    test_path = os.path.join(data_dir, 'test.triples.txt')
...
def train(lf):
    ...
    if args.adaptation:
        few_train_data = {k: few_train_data[k] for k in list(few_train_data)[:]}
        # for key, value in new_few_train_data.items():
        #     print (key)
...
def inference(lf):
    ...
    if args.compute_map:
        ...
        dev_path = os.path.join(args.data_dir, 'dev.triples.txt')
        dev_data = data_utils.load_triples(dev_path, entity_index_path, relation_index_path, seen_entities=seen_entities)
...
def compute_fact_scores(lf):
    data_dir = args.data_dir
    train_path = os.path.join(data_dir, 'train.triples.txt')
    dev_path = os.path.join(data_dir, 'dev.triples.txt')
    test_path = os.path.join(data_dir, 'test.triples.txt')
    entity_index_path = os.path.join(args.data_dir, 'entity2id.txt')
    relation_index_path = os.path.join(args.data_dir, 'relation2id.txt')
